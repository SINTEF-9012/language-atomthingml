...
const thingml = {
  ...
  callCompiler: (compiler, src, output) => {
    ...
    java.stdout.on('data', (data) => {
      if (data.startsWith('[WARNING]')) { //FIXME: Messages from the checker seem to arrive all in one line...
        thingml.warn(data);
      } else if (data.startsWith('[ERROR]')) {//ideally, that should arrive on stderr
        hasError = true;
        thingml.err(data);
      } else {
        thingml.out(data);
    ...
    java.stderr.on('data', (data) => {//FIXME: Data is actually a chunk, we should first split it in proper lines so that we can isolate errors.
      hasError = true;
      thingml.err(data);
      var fileRegex = /([a-zA-Z]:\\)?([a-zA-Z0-9_ \-\./]+\\)*[a-zA-Z0-9_ @\-\./]+\.thingml/i
      var lineRegex = /\(([0-9]*),/

      if (data.match(fileRegex).length > 0 && data.match(lineRegex).length > 1) {//we have a file and a line (most likely)
        thingml.errLine(data.match(fileRegex)[0], data.match(lineRegex)[1], data);
      }
    });
